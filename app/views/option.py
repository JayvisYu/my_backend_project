...
# 获取择时风险分析表
@option_router.get('/get_timing_risk_analysis_data', summary='获取择时风险分析表')
@login_require
...
# 获取择时资金曲线/最大回撤表
@option_router.get('/get_timing_curve_chart_data', summary='获取择时资金曲线/最大回撤表')
@login_require
...
# 获取择时历年收益表
@option_router.get('/get_timing_year_profit_data', summary='获取择时历年收益表')
@login_require
...
# 获取择时html数据
@option_router.get('/get_timing_kline_data', summary='获取择时开平仓图')
@login_require
async def get_timing_kline_data(request: Request, tactic_name: str):
    df = pd.read_csv(
        '/Users/jarvisyu/05_hobbies/Jarvis_Admin/backend/app/backtest/option/data/' + tactic_name + '.csv')
